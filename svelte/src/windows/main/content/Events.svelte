...
<main class="w-full overflow-y-auto">
	<h1 class="text-center mt-5">AutoEvent</h1>
	<div class="m-10 h-full">
		...
		{#if websocket && status}
			{#each $websocketMessages as msg}
				{#if msg._type !== "status"}
				<EventCard name={msg.eventSource.name} description={msg.message.data.cleanContent.substring(0, 100)} icon={msg.guild.iconUrl}/>
				{/if}
			{/each}
